...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  const cardStyle = useMemo(
    () => ({
      ...
      '--behind-glow-color': behindGlowColor ?? 'rgba(154, 57, 242, 0.4)',
  ...
  const handleContactClick = useCallback((): void => {
    if (contactUrl && contactUrl.startsWith('#')) {
      document.querySelector(contactUrl)?.scrollIntoView({ behavior: 'smooth' });
    } else if (contactUrl) {
      window.location.href = contactUrl;
    }
    onContactClick?.();
  }, [contactUrl, onContactClick]);
            ...
            >
              <img
                ...
                style={{
                  ...
                  transform:
                    'translateX(calc((var(--pointer-from-left) - 0.5) * 3px)) translateY(calc((var(--pointer-from-top) - 0.5) * 3px)) translateZ(0)',
                  borderRadius: cardRadius,
                  backfaceVisibility: 'hidden'
                }}
                onError={e => {
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col items-start gap-0.5">
                      <div className="text-xs font-medium text-white/90 leading-tight">@{handle}</div>
                      <div className="text-xs text-white/60 leading-tight">{status}</div>
                    </div>
                  </div>
                  <button
